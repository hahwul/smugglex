...
/// A regular HTTP/2 header supplied by the caller or generated by a probe.
...
fn parse_extra_headers(custom_headers: &[String], cookies: &[String]) -> Vec<H2Header> {
...
impl H2Request<'_> {
    fn header_block(&self) -> Vec<u8> {
        ...
        for header in &self.extra {
            encode_field(
                &mut b,
                &HField::NewName(header.name.as_bytes(), header.value.as_bytes()),
            );
        }
        b
    }
...
/// Parameters for the real-HTTP/2 downgrade smuggling check.
pub struct H2DowngradeParams<'a> {
    pub host: &'a str,
    pub port: u16,
    pub authority: &'a str,
    pub path: &'a str,
...
pub async fn run_h2_downgrade_check(params: H2DowngradeParams<'_>) -> CheckResult {
    let H2DowngradeParams {
        host,
        port,
        authority,
        path,
    ...
    let extra_headers = parse_extra_headers(custom_headers, cookies);
    let probe_method = if method.trim().is_empty() {
        "POST"
    } else {
        method
    };
    ...
    // we can speak HTTP/2 here and a reference latency. Caller headers and
    // cookies are included so authenticated virtual routes are tested.
    let baseline = H2Request {
        method: "GET",
        authority,
        path,
        extra: extra_headers.clone(),
        content_length: None,
        body: b"",
    };
    ...
    let mut h2_te_headers = extra_headers.clone();
    h2_te_headers.push(H2Header {
    ...
    let shapes = [
        ("h2.cl", "content-length: 50", h2_cl),
        ("h2.te", "transfer-encoding: chunked", h2_te),
    ];
    ...
    for (name, desc, attack_req) in &shapes {
        ...
        // Capture a well-formed response for the same method/path shape. A
        // GET baseline is useful for connectivity, but comparing a malformed
        // POST directly to GET would turn ordinary method routing into a
        // misleading response-set difference.
        let control_out = h2_probe(host, port, &control, false, dur).await;
        if attack.responded
            && control_out.responded
            && attack.status.is_some()
            && attack.status != control_out.status
        {
            response_diagnostics.push(format!(
                "h2_response_status_mismatch:{}:{}:{}",
                name,
                control_out
                    .status
        ...
        if !control_out.responded || stalled(&control_out, dur) {
            if verbose {
                println!(
                    "  [*] {} {} control did not produce a response — rejecting",
                    check_name, name
                );
            }
            continue;
        }
        // Send a second well-formed request with DATA after the malformed
        // probe. This confirmation is deliberately separate from the attack
        // connection: it verifies that a valid stream shape is still accepted
        // and that the observed stall is tied to the malformed framing.
        let confirmation_out = h2_probe(host, port, &control, false, dur).await;
        if !confirmation_out.responded {
            if verbose {
                println!(
                    "  [*] {} {} valid DATA confirmation was rejected",
                    check_name, name
                );
            }
            continue;
        ...
        let signals = vec![
            ...
            "confirmation_data_responds".to_string(),
            format!(
                "timing_anomaly:{:.1}x",
        ...
        let payload = format!(
            "HTTP/2 {} smuggling: :method {} :authority {} :path {} {} (declared body never sent, END_STREAM on HEADERS; a valid DATA confirmation responded)",
            name, probe_method, authority, path, desc,
        );
        return CheckResult {
            check_type: check_name.to_string(),
            vulnerable: true,
    ...
    not_vulnerable(normal_status, base_ms as u64, response_diagnostics)
}
...
mod tests {
    ...
    #[test]
    fn parse_extra_headers_includes_cookies() {
        let headers = parse_extra_headers(
            &[
                "X-Test: value".to_string(),
                "Authorization: Bearer token".to_string(),
            ],
            &["sid=abc".to_string(), "theme=dark".to_string()],
        );
        assert_eq!(
            headers,
            vec![
                ...
                H2Header {
                    name: "cookie".to_string(),
                    value: "sid=abc; theme=dark".to_string()
                }
            ]
        );
    }
