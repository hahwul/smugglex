# Bumps the smugglex version across every file that hardcodes it
...
#
# Pre-release suffixes are allowed (e.g. 0.3.0-dev.1, 0.3.0-rc.1).
...
# line (Cargo.toml's [package] block, the smugglex entry in Cargo.lock).
...
# snapcraft.yaml here keeps the bare X.Y.Z form (no `v` prefix).
def update_snap(new_version : String) : Bool
  content = File.read(SNAP_YAML)
  updated = content.sub(/^(version:\s*)['"]?v?[^'"\s]+['"]?\s*$/m, "\\1#{new_version}")
  return false if updated == content
